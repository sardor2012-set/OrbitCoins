...
async def callback_premium(callback: CallbackQuery):
    ...
    await callback.message.answer(
        ...
        'В среднем активные пользователи зарабатывают около 5 500 ₽ в месяц.\n'
        'Без Plus ты получаешь 10% от заработка реферала — это примерно 550+ ₽ с одного такого реферала.\n'
        'С OrbitCoins Plus ты получаешь уже 20% — около 1 100+ ₽ с одного реферала. <tg-emoji emoji-id="5438496463044752972">🤯</tg-emoji>\n'
        'Получается, что всего один активный реферал может принести тебе 1 100+ ₽, а подписка за 200 ₽ окупается практически сразу.\n\n'
